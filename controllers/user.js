...
###############################*/
  ...
  var var_num = 4; // will change it after aliison added their results
  var result = ["condition1","condition2","condition3", "condition4"][Math.floor(Math.random() * var_num)]
...
exports.getGuest = (req, res, next) => {
  ...
  group = req.params.caseId;
  const user = new User({
    password: "thinkblue",
    username: "guest"+makeid(10),
    group: group, //will need to assign it based on their condition.
    // can be done based on the url specific to their experimental condition..  the url can be sent in req
    active: true,
    ui: 'no', //ui or no
