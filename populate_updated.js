...
function createActorInstances() {           
    async.each(actors_list, function (actor_raw, callback) {
        ...
        actor.save(function (err) {
            if (err) {
                console.log("Something went wrong!!!", actor_raw.username);
                return -1;
            }
    ...
    async.each(posts_list, function (new_post, callback) {
        Actor.findOne({ username: new_post.actor }, (err, act) => {
            ...
            if (act) {  
                ...
                // postdetail.condition3 = new_post.condition3;
                // postdetail.condition4 = new_post.condition4;

                postdetail.ZA = new_post.ZA;
                ...
                script.save(function (err) {
                    if (err) {
                        console.log("Something went wrong in Saving POST!!!??", new_post.id);
                        // console.log(err);
                        callback(err);
                    }
